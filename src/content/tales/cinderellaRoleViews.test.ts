...
import { getRoleView } from './cinderellaRoleViews.ts'

test('every role-specific choice leads to a scene and each role can finish', () => {
  for (const role of ['whisk', 'visitor'] as const) {
    let scene = CINDERELLA.scenes[0]
    let flags = {}
    for (let step = 0; step < 3; step++) {
      ...
      scene = CINDERELLA.scenes.find((candidate) => candidate.id === next)!
      assert.ok(scene)
